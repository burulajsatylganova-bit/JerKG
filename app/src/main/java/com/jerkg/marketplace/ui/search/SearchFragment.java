// Файл: app/src/main/java/com/jerkg/marketplace/ui/search/SearchFragment.java
package com.jerkg.marketplace.ui.search;
...
import com.jerkg.marketplace.databinding.FragmentSearchBinding;
...
public class SearchFragment extends Fragment {
    ...
    private void setupCategoryChips() {
        addChip(binding.chipGroupCategories, "Все", null, true, true);
        for (Category c : CategoryHelper.getAllCategories()) {
            addChip(binding.chipGroupCategories, c.getNameRu(), c.getId(), false, true);
        }
    }

    private void setupRegionChips() {
        List<String> regions = CategoryHelper.getRegions();
        for (int i = 0; i < regions.size(); i++) {
            ...
            chip.setOnCheckedChangeListener((b, checked) -> {
                if (checked) {
                    selectedRegion = "Все регионы".equals(region) ? null : region;
                    loadAds();
                }
            });
    ...
    private void loadAds() {
        ...
        AdRepository.AdsCallback cb = new AdRepository.AdsCallback() {
            @Override public void onSuccess(List<Ad> ads) {
                ...
                binding.tvResultCount.setText("Найдено: " + ads.size());
