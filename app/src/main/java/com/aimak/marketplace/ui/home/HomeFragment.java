...
import com.aimak.marketplace.databinding.FragmentHomeBinding;
...
public class HomeFragment extends Fragment {
    ...
    public void onResume() {
        ...
        if (viewModel != null && getContext() != null) {
            // Передаём свежий Context из фрагмента — ViewModel может иметь устаревший
            viewModel.loadCategories(requireContext());
        }
    }

    private void setupRecyclerViews() {
        // Категории — горизонтальный список (заполняется через LiveData)
        ...
        adAdapter = new AdAdapter(new AdAdapter.OnAdClickListener() {
            @Override
            public void onAdClick(com.aimak.marketplace.data.model.Ad ad) {
            ...
            @Override
            public void onFavoriteClick(com.aimak.marketplace.data.model.Ad ad) {
                toggleFavorite(ad);
            }
        });
    ...
    private void showRegionDialog() {
        String[] regions = CategoryHelper.getRegions(getContext()).toArray(new String[0]);
    ...
    private void toggleFavorite(com.aimak.marketplace.data.model.Ad ad) {
