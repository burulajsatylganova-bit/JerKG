...
public class AdRepository {
    ...
    public void getMyAds(AdsCallback callback) {
        ...
        if (uid == null) {
            callback.onError(ResUtils.str(R.string.error_not_authorized));
            return;
        }
        ...
        db.collection("ads").document(adId).get()
                .addOnSuccessListener(doc -> {
                    if (doc.exists()) {
                        ...
                        callback.onError(ResUtils.str(R.string.error_ad_not_found_short));
                    }
                })
                .addOnFailureListener(e -> callback.onError(e.getMessage()));
    }
    ...
    /**
     * Обновить объявление — только изменённые поля, не трогая userId/active/createdAt.
     */
    public void updateAd(Ad ad, VoidCallback callback) {
        ...
        updates.put("negotiable",  ad.isNegotiable());
        updates.put("category",    ad.getCategory());
        updates.put("region",      ad.getRegion());
